...
router.get("/download/:path(*)", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .send("An error occurred while downloading the file from Supabase.");
    }
...
router.get("/list-files", authMiddleware, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .send("An error occurred while listing files from Supabase.");
    }
...
router.delete("/delete-file/:path(*)", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .send("An error occurred while deleting file from Supabase.");
    }
...
router.get("/buckets-list", authMiddleware, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .send("An error occurred while listing buckets from Supabase.");
    }
    res.json(data);
  } catch (error) {
